...
export class BrowserOSAdapter {
  ...
  private isAvailable(): boolean {
    // @ts-ignore
    return typeof chrome !== 'undefined' && !!chrome.browserOS
  }
  ...
  async getInteractiveSnapshot(
    ...
    if (!this.isAvailable()) throw new Error('BrowserOS API not available')
  ...
  async getVersion(): Promise<string | null> {
    return new Promise<string | null>((resolve, reject) => {
      if (!this.isAvailable() || typeof chrome.browserOS.getVersionNumber !== 'function') {
        resolve(null)
        return
      }
  ...
  async getBrowserosVersion(): Promise<string | null> {
    return new Promise<string | null>((resolve, reject) => {
      if (!this.isAvailable() || typeof chrome.browserOS.getBrowserosVersionNumber !== 'function') {
        resolve(null)
        return
      }
  ...
  async logMetric(
    ...
    if (!this.isAvailable() || typeof chrome.browserOS.logMetric !== 'function') {
      return
    }
  ...
  async executeJavaScript(tabId: number, code: string): Promise<any> {
    if (!this.isAvailable() || typeof chrome.browserOS.executeJavaScript !== 'function') {
  ...
  async clickCoordinates(tabId: number, x: number, y: number): Promise<void> {
    if (!this.isAvailable() || typeof chrome.browserOS.clickCoordinates !== 'function') {
  ...
  async typeAtCoordinates(
    ...
    if (!this.isAvailable() || typeof chrome.browserOS.typeAtCoordinates !== 'function') {
  ...
  async getPref(name: string): Promise<PrefObject> {
    if (!this.isAvailable() || typeof chrome.browserOS.getPref !== 'function') {
      // Return a dummy pref or throw? Throwing is safer for logic flow, but might crash UI.
      // Let's throw for now as expected behavior for missing API.
  ...
  async setPref(name: string, value: any, pageId?: string): Promise<boolean> {
    if (!this.isAvailable() || typeof chrome.browserOS.setPref !== 'function') {
  ...
  async getAllPrefs(): Promise<PrefObject[]> {
    if (!this.isAvailable() || typeof chrome.browserOS.getAllPrefs !== 'function') {
  ...
  isAPIAvailable(method: string): boolean {
    return this.isAvailable() && method in chrome.browserOS
  }
