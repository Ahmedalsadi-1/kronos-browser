...
export function useLlmProviders(): UseLlmProvidersReturn {
  ...
  const [defaultProviderId, setDefaultProviderId] =
    useState<string>('opencode-ai-zen') // Changed default to 'opencode-ai-zen'
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (!loadedDefaultId) {
          loadedDefaultId = 'opencode-ai-zen' // Changed fallback to 'opencode-ai-zen'
  ...
  const deleteProvider = async (providerId: string) => {
    ...
    const builtInProviders = createDefaultProvidersConfig().map(p => p.id);
    ...
    if (defaultProviderId === providerId) {
      const newDefaultId = updatedProviders[0]?.id || 'opencode-ai-zen'; // Fallback to 'opencode-ai-zen'
