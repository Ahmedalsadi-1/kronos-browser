...
const SuggestionItemRenderer: FC<{
  ...
  switch (item.type) {
    ...
    case 'ai-tab':
      return (
        <li className={baseClassName} {...getItemProps({ item, index })}>
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--accent-kronos)]/10 transition-colors group-hover:bg-[var(--accent-kronos)]/20">
            <item.icon className="h-4 w-4 text-[var(--accent-kronos)]" />
          </div>
      ...
      return (
        <li className={baseClassName} {...getItemProps({ item, index })}>
          ...
          <span className="font-semibold">
            {item.mode === 'chat' ? 'Ask KRONOS:' : 'Run Agent:'}
          </span>
