  ...
  return (
    ...
    >
      ...
      <textarea
        ref={textareaRef}
        className={cn(
          'field-sizing-content max-h-60 min-h-[52px] flex-1 resize-none overflow-hidden rounded-2xl border border-border/50 bg-muted/50 px-4 py-2.5 pr-11 text-sm outline-none transition-colors placeholder:text-muted-foreground/70 hover:border-border focus:border-[var(--accent-kronos)]',
        )}
        value={input}
      ...
      {status === 'streaming' ? (
        ...
        <button
          type="submit"
          disabled={!input.trim()}
          className="absolute right-1.5 bottom-1.5 cursor-pointer rounded-full bg-[var(--accent-kronos)] p-2 text-white shadow-sm transition-all duration-200 hover:bg-[var(--accent-kronos-bright)] disabled:cursor-not-allowed disabled:opacity-50"
        >
