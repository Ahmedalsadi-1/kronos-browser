...
import type { ChatConversation, ChatMode, Provider } from './chatTypes'
import { useNotifyActiveTab } from './useNotifyActiveTab'

export const chatHistoryStorage = storage.defineItem<ChatConversation[]>(
  'local:chat-history',
  {
    fallback: [],
  },
...
export const useChatSession = () => {
  ...
  const [pastChats, setPastChats] = useState<ChatConversation[]>([])

  // Load chat history on mount
  useEffect(() => {
    chatHistoryStorage.getValue().then((history) => {
      setPastChats(history || [])
      // Potentially load the latest conversation if desired
      if (history && history.length > 0) {
        // For now, we're just loading the list of past chats.
        // Loading a specific chat into the current session will be handled by loadPastChat.
      }
    })
  }, [])
  ...
  const resetConversation = () => {
    ...
    if (messages.length > 0) {
      const title = messages[0].parts.filter(part => part.type === 'text').map(part => part.text).join('').substring(0, 50); // Or generate a better title
      const newConversation: ChatConversation = {
        id: conversationIdRef.current,
        title: title || 'New Chat',
        messages: messages,
        selectedProviderId: selectedLlmProvider?.id || 'unknown',
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
      chatHistoryStorage.getValue().then((history) => {
        const updatedHistory = [...(history || []), newConversation];
        chatHistoryStorage.setValue(updatedHistory);
  ...
  const loadPastChat = (conversation: ChatConversation) => {
    // Set the current messages and selected provider based on the loaded conversation
    setMessages(conversation.messages);
    conversationIdRef.current = conversation.id;
    setDefaultProvider(conversation.selectedProviderId);
    // Reset likes/dislikes as they are specific to a session
    setLiked({});
    setDisliked({});
    setTextToAction(new Map()); // Reset actions
  };

  const deletePastChat = (conversationId: string) => {
    chatHistoryStorage.getValue().then((history) => {
